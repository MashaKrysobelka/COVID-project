...
class COVIDModel(pl.LightningModule):
    def __init__(self, num_classes=2, freeze_encoder=True):
        ...
        self.encoder.classifier[1] = nn.Linear(in_features, num_classes)  # Два выхода для двух классов
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        logits = self(x)  # Выход модели (batch_size, 2)
        loss = self.loss_fn(logits, y)  # Метки y имеют форму (batch_size,)
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch
        logits = self(x)  # форма (batch_size, 2)
        loss = self.loss_fn(logits, y) # форма (batch_size,)
        acc = self.accuracy(logits, y)
        probs = torch.softmax(logits, dim=1)  # вероятности
        auc = self.auroc(probs, y)
